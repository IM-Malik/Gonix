...
func AddServer(directoryPath string, domain string, listenPort int, proxyPass string, uri string, enableSSL bool, certPath string, keyPath string, httpOrhttps string) (string, error) {
	...
	if status {
		// _, err := AddLocation(directoryPath, domain, proxyPass, uri, httpOrhttps)
		// if err != nil {
...
// func AddLocation(directoryPath string, domain string, proxyPass string, uri string, httpOrHttps string) (string, error) {
